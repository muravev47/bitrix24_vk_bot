                                ...
                                if message.get("from_id", 0) > 0:
                                    ...
                                    for att in attachments:
                                        if att.get("type") == "audio_message":
                                            voice_url = att["audio_message"]["link_mp3"]
                                            break
                                    if voice_url:
                                        # Распознаём голос
                                        try:
                                            text = await self.recognizer.recognize(voice_url)
                                        except Exception as e:
                                            await self.send_message(user_id, "Не удалось распознать голосовое сообщение.")
                                            continue
                                    if not text.strip():
                                        continue
